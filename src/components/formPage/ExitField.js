...
function ExitField(props) {
    const handleExitChecked = (event) => {
        props.exitCheckedChange(event.target.checked);
    }
    ...
    return (
        <div>
            ...
            <div style={{ display: "flex" }}>
                <div className="menu-sub-text">출구버튼링크</div>
                <div className="menu-right-align">
                    <Switch color="primary" checked={props.data.exitChecked} onChange={handleExitChecked} />
                </div>
                { props.data.exitChecked && 
                    <div style={{ position: "fixed", paddingTop: "40px", paddingLeft: "20px" }}>
